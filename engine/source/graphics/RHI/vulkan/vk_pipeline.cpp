...
namespace cannele::inline graphics::rhi::vk
{
        ...
        : VulkanDeviceChild<VulkanGraphicsPipeline>(device)
    {
        ...
        if (vulkan_ps_module) {
            ...
            shader_stage_ci->module = vulkan_ps_module->shader_module;
            shader_stage_ci->pName  = vulkan_ps_module->entry_point.c_str();

            push_constant_size = std::max(push_constant_size, vulkan_ps_module->push_constant_size);
            shader_stage_flags |= vulkan_ps_module->stage;
        }
