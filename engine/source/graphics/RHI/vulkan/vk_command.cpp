...
namespace cannele::inline graphics::rhi::vk
{
    ...
    {
        vkDestroyCommandPool(parent->device, command_pool, parent->allocator_callbacks);
    ...
    VulkanCommandList::VulkanCommandList(VulkanDevice* device, CommandListCreateInfo* info)
        : VulkanDeviceChild<VulkanCommandList>(device), info(*info)
    {
    ...
    auto VulkanCommandList::write_buffer(BufferHandle buffer, std::span<std::byte> data, size_t offset_bytes) -> void
    {
        ...
        if (data.size() <= upload_buffer_limit && (offset_bytes & 3) == 0) {
            ...
            // TODO: use staging buffer to upload.
        }
    }
    ...
    auto VulkanCommandList::set_graphics_state(GraphicsState* state) -> void
    {
        ...
        if(auto render_target = state->render_target) {
            ...
            if (auto attachment = render_target->depth_stencil_attachment) {
                vkCmdSetDepthTestEnable(active_command_buffer->command_buffer, render_target->info.depth_state.enable_depth_test ? VK_TRUE : VK_FALSE);
                vkCmdSetDepthWriteEnable(active_command_buffer->command_buffer, render_target->info.depth_state.enable_depth_write ? VK_TRUE : VK_FALSE);
                ...
                vkCmdSetDepthCompareOp(active_command_buffer->command_buffer, convert_to_vk_compare_op(render_target->info.depth_state.depth_compare));
            } else {
                vkCmdSetDepthTestEnable(active_command_buffer->command_buffer, VK_FALSE);
                vkCmdSetDepthWriteEnable(active_command_buffer->command_buffer, VK_FALSE);
                vkCmdSetDepthBoundsTestEnable(active_command_buffer->command_buffer, VK_FALSE);
                vkCmdSetDepthBiasEnable(active_command_buffer->command_buffer, VK_FALSE);
                vkCmdSetDepthClampEnableEXT(active_command_buffer->command_buffer, VK_FALSE);
                vkCmdSetDepthCompareOp(active_command_buffer->command_buffer, VK_COMPARE_OP_ALWAYS);
            }
    ...
    // Synchronization
    auto VulkanCommandList::commit_barriers() -> void
    {
        ...
        // TODO: use ranges::.
        for (auto& barrier: resource_state_tracker.buffer_barriers) {
            auto vulkan_buffer = (VulkanBuffer*) barrier.buffer;
            auto buffer_barrier = &vk_buffer_barriers.emplace_back(VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER_2);
            buffer_barrier->buffer              = vulkan_buffer->buffer;
            buffer_barrier->size                = vulkan_buffer->info.size_bytes;
            buffer_barrier->srcStageMask        = pipeline_stage_from_access(barrier.src_state); // TODO: improve.
            buffer_barrier->dstStageMask        = pipeline_stage_from_access(barrier.dst_state);
            buffer_barrier->srcAccessMask       = convert_to_vk_access_type(barrier.src_state);
            buffer_barrier->dstAccessMask       = convert_to_vk_access_type(barrier.dst_state);
        ...
        for (auto& barrier: resource_state_tracker.texture_barriers) {
            auto vulkan_texture = (VulkanTexture*) barrier.texture;
            auto image_barrier = &vk_image_barriers.emplace_back(VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER_2);
            image_barrier->image               = vulkan_texture->image;
            image_barrier->oldLayout           = image_layout_from_access(barrier.src_state, is_depth_stencil_format(convert_to_vk_format(vulkan_texture->info.format)));
            image_barrier->newLayout           = image_layout_from_access(barrier.dst_state, is_depth_stencil_format(convert_to_vk_format(vulkan_texture->info.format)));
            image_barrier->srcStageMask        = pipeline_stage_from_access(barrier.src_state);
            image_barrier->dstStageMask        = pipeline_stage_from_access(barrier.dst_state);
            image_barrier->srcAccessMask       = convert_to_vk_access_type(barrier.src_state);
            image_barrier->dstAccessMask       = convert_to_vk_access_type(barrier.dst_state);
